...
function IRSend (config) {
  backendParameters = getBackendParameters(config || {});
};
...
function getBackendParameters(config) {
  var parameters = [];

  if (config.address || config.port) {
    var address = [config.address, config.port].filter(function(item) {return item}).join(':');
